...
void OgreNode::setOgreEngine(OgreEngine *ogreEngine)
{
    m_ogreEngine = ogreEngine;
}
...
void OgreNode::activateOgreContext()
{
    ...
    m_ogreEngine->ogreContext()->functions()->glBindFramebuffer(GL_FRAMEBUFFER_EXT, m_ogreFboId);
}
...
void OgreNode::updateFBO()
{
    ...
    int samples = m_ogreEngine->ogreContext()->format().samples();
    ...
    delete m_texture;
    m_texture = m_ogreEngine->createTextureFromId(nativeTexture->getGLID(), m_size);
